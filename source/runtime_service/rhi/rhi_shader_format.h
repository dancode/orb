...
/*==============================================================================================
    ...
    pair).  File layout -- a fixed 72-byte header followed by six tightly packed sections:

        [ oshd_header_t ]                         opens with res_ref_head_t (res_ref.h)
        [ refs              ref_size bytes    ]   resources the content names: a padded
                                                  string table, 8-aligned (RES_REF_ALIGN)
    ...
    A shader names no other resource, so every cooked .oshd has ref_count 0 and ref_size 0 and
    readers step over an empty section.  All fields are little-endian (tool and engine share
    the architecture).  strtab_size is padded to a multiple of 4 so the SPIR-V payload stays
    ...
    Versions:
        4  header opens with res_ref_head_t; the reference section is a padded string table.
        3  ref_count + a reference section of u32 ids between header and inputs.
...
typedef struct oshd_header_s
{
    ...
    u32 ref_count;         // names in the reference section (res_ref.h); 0 -- a shader names nothing
    u32 ref_size;          // padded bytes of the reference section; 0 today
    u32 ref_offset;        // where the section starts: sizeof( oshd_header_t )
...
RES_REF_HEAD_ASSERT( oshd_header_t );

/* The reference section is padded to RES_REF_ALIGN (8), so the oshd_pc_member_t records (u64
   name_hash) stay aligned in a mapped file. */
_Static_assert( sizeof( oshd_header_t ) % 8 == 0, "oshd header must keep the sections 8-aligned" );
