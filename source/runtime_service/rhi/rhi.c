/*==============================================================================================
    ...
    -----------------------
       ...
       16. vk_pipeline_cache.c     (VkPipelineCache disk persistence)
       17. vk_pipeline_graphics.c  (slot helpers + graphics PSO creation)
       18. vk_pipeline_compute.c   (compute PSO creation)
       19. vk_upload.c             (staged upload ring buffer + QFOT)
       20. vk_instance.c           (VkInstance + extensions + layers)
       21. vk_swapchain.c          (VkSurfaceKHR + VkSwapchainKHR + depth buffer)
       22. vk_device_select.c      (physical device enumeration + feature validation)
       23. vk_device.c             (VkDevice creation + queue retrieval + subsystem init)
       24. vk_sync.c               (per-frame semaphores + fences)
       25. vk_command.c            (command pool + per-frame command buffers)
