/*==============================================================================================
    ...
    runtime_service/gui/2_compose/gui_layout_core.c -- Layout mechanism: track resolver + cell emitters.
...
/* layout_push_scoped / layout_pop_scoped -- a minimal layout-frame push/pop at an explicit
   (x, y, w), used only by the volatile-widget replay scope, now live in 3_widgets/gui_volatile.c
...
line_place_pen( layout_frame_t* f, f32 natural_w, f32 h )
{
    ...
    f32 main_avail = ( horiz ? ( f->content_x + f->content_w ) : f->band_bottom ) - f->line.main;
    if ( main_avail < 0.0f ) main_avail = 0.0f;

    f32 main_ext      = unit_resolve( f->line.pack_size_next, nat_main, main_avail );
