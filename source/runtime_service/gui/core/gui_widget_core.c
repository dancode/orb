/*==============================================================================================
    ...
    runtime_service/gui/core/gui_widget_core.c -- Shared presentation primitives.
    ...
    interact/gui_item.c, included immediately after this file so it can use the WIN_BORDER /
    COL_NAV macros for the nav focus ring.  The shared edge-resize geometry is
    interact/gui_resize.c and the layout engine (track resolver + cell emitters) is
    compose/gui_layout_core.c.

    Included by gui.c after core/gui_ctx.c + interact/gui_io.c so s_interaction, s_build, s_io, s_style,
...
/* The symbol render primitives -- the glyph marks (arrow / collapse arrow / check / bullet / close /
   ...
   included later in Tier 0a (after the interact/ services) so they may use the COL_* / ROUND_* /
