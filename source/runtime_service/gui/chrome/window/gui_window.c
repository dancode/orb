/*==============================================================================================
    ...
    runtime_service/gui/chrome/window/gui_window.c -- Window gesture policy state.
    ...
    raise, floating groups raise as a node -- needs dock/ knowledge the surface tier must
    not have).

    Drag / resize / scrollbar interaction lives in gui_window_free.c alongside
...
/* Click-vs-drag disambiguation for the native title bar and the CAN_AUTOSIZE corner grip --
   ...
   can be tested.  The threshold latch is the deferred-press service (interact/gui_move.c):
