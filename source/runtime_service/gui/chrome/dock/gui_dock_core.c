...
static void
dock_splitter( gui_dock_node_t* n, i32 vp )
{
    gui_rect_t r     = n->rect;
    f32          thick = DOCK_SPLITTER;
    gui_rect_t sr;
...
static void
dock_tree_splitters( gui_dock_node_t* n, i32 vp )
{
...
dock_tree_placeholders( gui_dock_node_t* n )
{
    ...
    if ( n->split == GUI_DOCK_SPLIT_NONE )
    {
        if ( n->tab_count == 0 )
        {
            ...
            draw_push_rect_filled ( n->rect.x, n->rect.y, n->rect.w, n->rect.h, 0, 0, 1, 1, 0, COL_PANEL_INERT );
            draw_push_rect_outline( n->rect.x, n->rect.y, n->rect.w, n->rect.h, WIN_BORDER, COL_BORDER_INERT );
        }
        return;
    }
