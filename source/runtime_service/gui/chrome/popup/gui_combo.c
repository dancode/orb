/*==============================================================================================
    ...
    runtime_service/gui/chrome/popup/gui_combo.c -- Combo box + list box.
    ...
    Two selection widgets built entirely out of the existing toolkit rather than new machinery:
      ...
      Combo   -- a framed preview box (selected text + a down arrow on the right edge) with a
                 ...
                 dropdown is the popup layer (gui_popup.c) keyed off the combo's widget id and
...
#define GUI_COMBO_POPUP_FLAGS \
    ( GUI_WIN_NOTITLEBAR | GUI_WIN_NOMOVE | GUI_WIN_NORESIZE | GUI_WIN_NOCOLLAPSE )
...
static i32
combo_cap_items( gui_combo_flags_t flags )
{
    if ( flags & GUI_COMBO_HEIGHT_SMALL   ) return 4;
    if ( flags & GUI_COMBO_HEIGHT_REGULAR ) return 8;
    if ( flags & GUI_COMBO_HEIGHT_LARGE   ) return 20;
    if ( flags & GUI_COMBO_HEIGHT_LARGEST ) return 0;   /* no cap: as many as fit */
...
gui_combo_begin( const char* label, const char* preview_value, gui_combo_flags_t flags )
{
    ...
    /* The box takes the control track and the label trails it (or sits in its field-split track),
       ...
       (gui_field_row), which also drops the label when hidden / skipped. */
    gui_field_row( label );
    ...
    gui_combo_state_t* cs  = GUI_STATE( gui_combo_state_t, id );
    ...
    draw_arrow( arrow, GUI_DIR_DOWN, COL_TEXT_PRIMARY_IDLE );
    ...
    if ( preview_value && preview_value[ 0 ] )
    {
        ...
        draw_text_fit_n( box.x + WIDGET_PAD, text_center_y( box.y, box.h ),
                         COL_TEXT_PRIMARY_IDLE, preview_value, 0xFFFFFFFFu, avail );
    }
    ...
    if ( cap > 0 )
    {
        vis = popup_begin_common_id( pid, NULL, GUI_COMBO_POPUP_FLAGS, false,
                                     box.w, combo_rows_h( cap ) );
    }
    else
    {
        vis = popup_begin_common_id( pid, NULL,
                                     GUI_COMBO_POPUP_FLAGS | GUI_WIN_ALWAYS_AUTOSIZE, false,
                                     0.0f, 0.0f );
    }
...
static struct { const char* label; gui_rect_t box; } s_listbox_stack[ LISTBOX_DEPTH ];
...
gui_listbox_begin( const char* label, f32 w, f32 h )
{
    ...
    gui_stack();   /* the list body is a vertical stack of selectable rows */
    return true;
}

void
gui_listbox_end( void )
{
    gui_child_end();
    ...
    if ( label_vis_len( label ) > 0 )
        draw_label( box.x + box.w + WIDGET_PAD, text_center_y( box.y, WIDGET_H ), COL_TEXT_PRIMARY_IDLE, label );
}
...
bool
gui_listbox( const char* label, i32* current_item, const char* const items[], i32 count,
               i32 height_in_items )
{
