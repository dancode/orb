/*==============================================================================================
    ...
    runtime_service/gui/4_table/gui_table.c -- Table layout: multi-column rows over the columns engine.
...
static gui_table_persist_t*
table_persist_get( gui_id_t id )
{
    u32 cur_frame = g_ctx->retained.frame;

    /* Prefer an exact match. */
    for ( i32 i = 0; i < GUI_TABLE_POOL_CAP; ++i )
    {
        if ( s_tpool[ i ].id == id )
        {
    ...
    for ( i32 i = 0; i < GUI_TABLE_POOL_CAP; ++i )
    {
        ...
        {
            gui_table_persist_t* p = &s_tpool[ i ];
            memset( p, 0, sizeof( gui_table_persist_t ) );
            p->id         = id;
...
table_header_interact( gui_table_t* t )
{
    ...
    {
        gui_table_col_t* col = ( i < t->col_setup_n ) ? &t->cols[ i ] : NULL;
        bool no_sort = !sortable || ( col && ( col->flags & GUI_TABLE_COL_NO_SORT ) );
