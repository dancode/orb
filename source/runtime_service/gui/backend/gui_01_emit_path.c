/*==============================================================================================
    ...
    runtime_service/gui/backend/gui_01_emit_path.c -- Line and path stroking.

    Builds antialiased / pixel-snapped stroke geometry on top of the raw draw-list reservation
    (draw_prim_begin in gui_01_emit_draw.c).  Two layers:
    ...
    Included by gui_backend.c immediately after gui_01_emit_draw.c (uses s_draw, draw_prim_begin/commit,
...
static f32
stroke_center_offset( gui_stroke_align_t align, f32 half )
{
    switch ( align )
    {
        case GUI_STROKE_INSIDE:  return  half;
        case GUI_STROKE_OUTSIDE: return -half;
...
stroke_axis_aligned_rect( f32 x0, f32 y0, f32 x1, f32 y1, f32 t,
                          gui_stroke_align_t align, u32 abgr )
{
    bool horiz = ( y0 == y1 );
    bool vert  = ( x0 == x1 );
    if ( !horiz && !vert )
        return false;
    if ( align != GUI_STROKE_CENTER_BIASED && align != GUI_STROKE_CENTER )
        return false;

    bool biased = ( align == GUI_STROKE_CENTER_BIASED );
...
/* Copy points into the per-frame point pool and push a CMD_POLYLINE semantic command.
   The pool (s_draw.points[]) is stable until gui_render_flush, where tess_dispatch reads it. */
static void
...
void
gui_draw_polyline( const gui_vec2_t* pts, u32 count, f32 thickness,
                     gui_stroke_align_t align, bool closed, u32 abgr )
{
...
void
gui_draw_dashed_line( f32 x0, f32 y0, f32 x1, f32 y1, f32 dash, f32 gap, f32 thickness, u32 abgr )
{
...
void
gui_path_line_to( f32 x, f32 y )
{
    if ( s_path.count < GUI_PATH_MAX )
        s_path.pts[ s_path.count++ ] = v2( x, y );
}

void
gui_path_stroke( f32 thickness, gui_stroke_align_t align, bool closed, u32 abgr )
{
    if ( s_path.count >= 2 )
