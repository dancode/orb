/*==============================================================================================
    ...
    runtime_service/gui/gui_render_tess.c -- CPU-side tessellation engine.
    ...
    s_tess is read only by the two files included immediately after: gui_render_cache.c (the
    ...
    Included by gui_backend.c after gui_draw_path.c (provides v2, seg_normal,
    stroke_center_offset, STROKE_* constants) and before gui_render_cache.c (which drives
...
/*----------------------------------------------------------------------------------------------
    Tessellation helpers -- mirrors of the draw_push_* functions in gui_draw.c, but writing
...
/*----------------------------------------------------------------------------------------------
    ...
    Mirrors the old stroke_poly_aa (removed from gui_draw_path.c in step 4) but writes into
    s_tess via tess_prim_begin/commit.  abgr is pre-baked (alpha folded in at emit time).
    v2 / seg_normal / stroke_center_offset are defined in gui_draw_path.c (included before
