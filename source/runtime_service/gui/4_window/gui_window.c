/*==============================================================================================
    ...
    runtime_service/gui/4_window/gui_window.c -- Persistent per-window state.
    ...
    (4_dock/gui_dock_serialize.c); a free window's geometry persists in memory only.
...
/*----------------------------------------------------------------------------------------------
    ...
    Thin alias for gui_hover_nominate (gui_region.c), which holds the actual body -- relocated
    there so a root region (compiled before this file, no 4_window/ dependency) can enter the same
    hover_win contest a window does.
----------------------------------------------------------------------------------------------*/

static void
window_nominate_hover( gui_id_t id, gui_rect_t r, u32 z, u32 viewport )
{
    gui_hover_nominate( id, r, z, viewport );
}
