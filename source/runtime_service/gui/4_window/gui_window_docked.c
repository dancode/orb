/*==============================================================================================
    ...
    runtime_service/gui/4_window/gui_window_docked.c -- The docked branch of window_begin.
    ...
    window_end via dock_window_chrome), and only the node's active tab opens a body.  Drag,
    edge-resize, tear-off, collapse and boundary-clamp are all skipped -- the node and its
    splitters own placement.

    Split out of gui_widget_window.c (the free-float path) because docked placement is a
    genuinely separate concern from it, sharing only the s_build.win / s_scope handoff both
    paths commit for window_end to read.  Mirrors the same-kind separation 4_dock/ already
    keeps between its own core/drag/float/serialize files.

    Included by gui.c right before gui_widget_window.c, whose window_begin_ex calls
