/*==============================================================================================
    ...
    and the per-widget interaction state machine.  Both the leaf widgets (gui_widget.c) and
    the window chrome (gui_widget_window.c) draw and interact through these, so they live
    here, ahead of both in the unity build.  The shared edge-resize geometry is gui_resize.c
    and the layout engine (track resolver + cell emitters) is gui_layout_core.c, both next.
...
/* Each resolves through style_var (gui_style.c): the font-derived base with any push_style_var /
   ...
   change.  See gui_style_var_t for the slots. */
...
/* Corner-radius categories, fed to draw_set_rounding (gui_draw.c) so a draw site can pick the
...
/* The symbol render primitives -- the glyph marks (arrow / collapse arrow / check / bullet / close /
   ...
   gui_render_* surface over them is centralized there too. */
