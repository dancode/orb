/*==============================================================================================
    ...
    runtime_service/gui/5_user/gui_stacks.c -- Bracketing vocabulary: id scope, item flags, style.

    The thin public wrappers for the three push / pop / next stacks a caller brackets widgets with:
        push_id / pop_id           -- id-scope levels for repeated widgets (id stack, 0_foundation/gui_id.c)
        push_item_flag / next_     -- per-item behavior tweaks (item-flag stack, 0_foundation/gui_ctx.c)
        push_style_color / _var    -- per-item theme overrides (style stacks, 0_foundation/gui_style.c)
    ...
    live with 0_foundation/; the verbs a user speaks live here).  Nothing in the lib below
    ...
    Included by gui.c in the 5_user/ tier (last of the tiers), before gui_api.c, which wires
