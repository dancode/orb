/*==============================================================================================
    ...
    The PUBLIC font surface (load / load_mem / get / load_into / use / push / pop / active_id),
    the push/pop font stack, and gui_style_apply -- the font -> layout bridge that rebuilds the
...
/* A load activates the new font: rescale layout to it and stamp it onto subsequent text. */
static u32
font_loaded( u32 id )
{
    if ( id == 0 )
...
u32
gui_font_load( const char* name )
{
    return font_loaded( font_load( name ) );     // name + ".orb_font" through the fs mounts
}

u32
gui_font_load_mem( const void* data, u32 size, const char* name )
{
    return font_loaded( font_load_mem( data, size, name ) );
}

/* Resolve a font by request -- family + pixel size -- WITHOUT activating it.  The resolver
   reads the cooked bake, asks the installed baker, or degrades to the nearest resident
   in-family size (warn-once); apply the id with font_use / push_font.  Retention is
   IMMEDIATE-MODE: call this every frame the font is in use (steady-state it is a memo probe),
   and the request itself is the hold.  A font not requested for an emitted frame goes stale
   and its slot may be reclaimed under registry pressure -- do NOT park the id in a static and
   stop asking; re-requesting after a lapse just reloads.  Never 0-fails into nothing: worst
   case the answer is the default font, id 0. */

u32
gui_font_get( const char* family, u32 size_px )
{
    u32 landed = 0;
    return font_resolve( family, size_px, true, &landed );
}

/* font_load_into parses the bake into the slot now (metrics live immediately); a slot that
...
bool
gui_font_load_into( u32 id, const char* name )
{
    return font_load_into( id, name );
}

bool
gui_font_load_into_mem( u32 id, const void* data, u32 size, const char* name )
{
    return font_load_into_mem( id, data, size, name );
}
