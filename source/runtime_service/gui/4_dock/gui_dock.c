/*==============================================================================================
    ...
    runtime_service/gui/4_dock/gui_dock.c -- Docking: the public build API.
...
gui_dockspace_over_viewport( gui_vp_t vp, gui_dockspace_flags_t flags )
{
    ...
    if ( !g_ctx->dock_nodes ) return GUI_DOCK_NONE;   /* pool disabled for this context (max_dock_nodes == 0) */
    if ( vp >= g_ctx->max_viewports )
...
{
    if ( !g_ctx->dock_nodes ) return GUI_DOCK_NONE;
    if ( vp >= g_ctx->max_viewports )
        return GUI_DOCK_NONE;
