/*==============================================================================================
    ...
    5_user/gui_query.c, included in the 5_user/ tier below.
...
/* True only while a volatile-widget callback is replayed standalone on an idle frame; set/cleared
   by gui_replay_scope_enter/_exit (3_widgets/gui_volatile.c).  Ambient frame-phase state, same tier as
   hover_id/active_id: widget_behavior (2_interact/gui_item.c) reads it inline to short-circuit before any
...
/*----------------------------------------------------------------------------------------------
    ...
    drives it; nav_item_register (2_interact/gui_item.c) is the per-item seam.
----------------------------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------------------------
    ...
    contract live in 0_foundation/gui_state.c, next to the id system that keys it (0_foundation/gui_id.c).
----------------------------------------------------------------------------------------------*/
...
/* Resolve an app win_id to the primary context's viewport index.  OS windows and their viewport
   ...
   Forward-declared in gui_internal.h; called by the mouse-input path in 0_foundation/gui_io.c. */
static u32
...
interaction_frame_reset( void )
{
    /* Snapshot the active item before this frame mutates it -- the previous-frame baseline the
       is_item_activated / is_item_deactivated edge readers compare against (5_user/gui_query.c). */
