/*==============================================================================================
    ...
    _rect_outline / _text, imgui_draw_line / imgui_draw_polyline) into named marks -- they draw
    through the normal vertex pipeline, NOT the runtime icon atlas (imgui_icon.c).  Two routes do
...
static u32
sym_arc( f32 cx, f32 cy, f32 r, f32 a0, f32 a1, imgui_vec2_t* out )
{
...
static void
fill_convex( const imgui_vec2_t* pts, u32 n, u32 col )
{
    for ( u32 i = 1; i + 1 < n; ++i )
...
static void
draw_chevron( imgui_rect_t box, imgui_dir_t dir, f32 thickness, u32 color )
{
    ...
    switch ( dir )
    {
        case IMGUI_DIR_LEFT:  p[0]=sv2(cx+s,cy-s); p[1]=sv2(cx-s,cy);   p[2]=sv2(cx+s,cy+s); break;
        case IMGUI_DIR_RIGHT: p[0]=sv2(cx-s,cy-s); p[1]=sv2(cx+s,cy);   p[2]=sv2(cx-s,cy+s); break;
        case IMGUI_DIR_UP:    p[0]=sv2(cx-s,cy+s); p[1]=sv2(cx,  cy-s); p[2]=sv2(cx+s,cy+s); break;
        case IMGUI_DIR_DOWN:  p[0]=sv2(cx-s,cy-s); p[1]=sv2(cx,  cy+s); p[2]=sv2(cx+s,cy-s); break;
        default:              return;
    }
    imgui_draw_polyline( p, 3, thickness, IMGUI_STROKE_CENTER, false, color );
}
...
/* Directional arrow glyph: a filled triangle (default) or a stroked chevron pointing `dir`,
   ...
   IMGUI_VAR_ARROW_STYLE (the chevron variant) exactly as check / bullet follow their style vars.
...
{
    if ( style_var( IMGUI_VAR_ARROW_STYLE ) >= 0.5f )
    {
...
static void
draw_plus_minus( imgui_rect_t box, bool plus, f32 thickness, u32 color )
{
    f32 cx = box.x + box.w * 0.5f;
    f32 cy = box.y + box.h * 0.5f;
...
static void
draw_frame( imgui_rect_t r, u32 col_bg, u32 col_border, f32 border )
{
...
static u32
round_rect_perimeter_ex( imgui_rect_t b, f32 rtl, f32 rtr, f32 rbr, f32 rbl, imgui_vec2_t* out )
{
...
{
    imgui_vec2_t pts[ 4 * 17 + 4 ];
    u32 n = round_rect_perimeter_ex( b, rtl, rtr, rbr, rbl, pts );
    if ( filled )
        fill_convex( pts, n, col );
    else
        imgui_draw_polyline( pts, n, thickness < 1.0f ? 1.0f : thickness, IMGUI_STROKE_CENTER, true, col );
}
...
draw_ngon( f32 cx, f32 cy, f32 r, u32 sides, f32 rot, bool filled, f32 thickness, u32 col )
{
    ...
    if ( filled )
        ...
        imgui_draw_polyline( pts, sides, thickness < 1.0f ? 1.0f : thickness, IMGUI_STROKE_CENTER, true, col );
}
...
draw_circle( f32 cx, f32 cy, f32 r, bool filled, f32 thickness, u32 col )
{
    ...
    imgui_draw_polyline( pts, segs, thickness < 1.0f ? 1.0f : thickness, IMGUI_STROKE_CENTER, true, col );
}
...
draw_arc( f32 cx, f32 cy, f32 r, f32 a0, f32 a1, f32 thickness, u32 col )
{
    ...
    imgui_draw_polyline( pts, n, thickness < 1.0f ? 1.0f : thickness, IMGUI_STROKE_CENTER, false, col );
}
...
{
    imgui_vec2_t pts[ SYM_BEZIER_SEGS + 1 ];
    ...
    imgui_draw_polyline( pts, SYM_BEZIER_SEGS + 1, thickness < 1.0f ? 1.0f : thickness,
                         IMGUI_STROKE_CENTER, false, col );
}
...
static void
draw_checker( imgui_rect_t box, f32 cell, u32 col_a, u32 col_b )
{
...
static void
draw_hatch( imgui_rect_t box, f32 spacing, f32 thickness, u32 col )
{
    ...
    for ( f32 x = box.x - box.h; x < end && guard < 512; x += spacing, ++guard )
        imgui_draw_line( x, box.y, x + box.h, box.y + box.h, thickness, col );
    draw_pop_clip_rect();
}
...
static void
draw_shadow( imgui_rect_t box, f32 spread, u32 col )
{
    enum { LAYERS = 6 };
...
static void
draw_grip_dots( imgui_rect_t box, u32 col )
{
...
static void
draw_spinner( imgui_rect_t box, f32 t, f32 thickness, u32 col )
{
...
void imgui_set_bullet_style( u32 style ) { s_layout.bullet_style = style; }
void imgui_set_arrow_style ( u32 style ) { s_layout.arrow_style  = style; }
