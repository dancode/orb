/*==============================================================================================
    ...
    runtime_service/imgui/imgui_icon.c -- Runtime icon atlas.
    ...
    an imgui_icon_id_t.  Drawing reuses the existing textured-quad primitive (draw_push_rect_filled)
    ...
    Included by imgui_backend.c after imgui_font.c.
...
icon_register( const char* name, u32 w, u32 h, const u8* coverage )
{
    ...
    {
        printf( "[imgui] icon atlas full -- '%s' (%ux%u) rejected\n", name ? name : "?", w, h );
        return IMGUI_ICON_NONE;
    }
    ...
    s_icons.dirty = true;
    return (imgui_icon_id_t)( ++s_icons.count );   /* id = (index + 1); 0 stays reserved for none */
}
...
icon_entry( imgui_icon_id_t id )
{
    if ( id == IMGUI_ICON_NONE || id > s_icons.count )
        return NULL;
...
icon_find( const char* name )
{
    ...
    for ( u32 i = 0; i < s_icons.count; ++i )
        if ( strcmp( s_icons.entries[ i ].name, name ) == 0 )
            return (imgui_icon_id_t)( i + 1 );
    return IMGUI_ICON_NONE;
}

bool
icon_get( imgui_icon_id_t id, f32* u0, f32* v0, f32* u1, f32* v1, u32* w, u32* h )
{
...
void
draw_push_icon( f32 x, f32 y, f32 w, f32 h, imgui_icon_id_t id, u32 abgr )
{
    const icon_entry_t* e = icon_entry( id );
    if ( !e )
        return;
