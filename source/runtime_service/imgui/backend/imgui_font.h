/*==============================================================================================
    ...
    runtime_service/imgui/backend/imgui_font.h -- Font types shared across the font unit.

    The font unit is split into three translation units, all included by imgui_backend.c:

        imgui_font.c      -- neutral: the id-addressed registry, slot lifecycle, glyph dispatch,
                             and the shared atlas finalize (white texel + dash rows + UV metrics).
        imgui_font_bmp.c  -- bmp fonts: a monospace R8 grid atlas.  The built-ins are 1-bpp "bit
                             fonts" (compile-time, bit_font_def_t) expanded into a bmp atlas.
        imgui_font_ttf.c  -- ttf fonts: a proportional .orb_font loaded at runtime.
    ...
    and IMGUI_DASH_PATTERN_COUNT stipple rows are appended so the active font alone backs solid
...
/* Capacity of the loaded-font registry (imgui_font.c).  Slot 0 is the default / fallback; loaded
   fonts occupy ids 1..IMGUI_FONT_REGISTRY_MAX-1. */
...
/* bmp source: glyph layout comes from the source bit font's grid.  The atlas itself is owned by
   imgui_font_bmp.c (the resident built-in set), so a slot referencing it never owns the GPU. */
typedef struct
{
...
void bmp_select          ( imgui_font_t font );
