/*==============================================================================================
    ...
    runtime_service/imgui/imgui_render_cache.c -- Retained frame-geometry cache (BUILD phase).

    The render pipeline has three phases.  This file is the middle one:

        EMIT   (imgui_draw.c)        widgets push semantic shapes -> s_draw command list,
        ...
        SUBMIT (imgui_render.c)      once per surface: upload the slots this surface owns and emit
    ...
    produces is identical no matter which surface triggers it.  imgui_render_frame_reset clears the
    guard at frame_begin.

    Included by imgui_backend.c after imgui_render_tess.c (s_tess, tess_reset, tess_dispatch) and
    imgui_draw.c (s_draw, fnv1a) -- both are read here -- and before imgui_render.c, whose flush
    consumes the dispatch table this file builds.

==============================================================================================*/
#include "runtime_service/imgui/imgui_internal.h"   // imgui_id_t, imgui_rect_t, IMGUI_MAX_* capacities
// clang-format off
...
// imgui_render_frame_reset -- clear the guard (frame_begin, right after draw_reset, before emit).
void
...
static struct
{
    imgui_render_stats_t accum;          // built across this frame's BUILD + flush(es)
    imgui_render_stats_t published;      // last frame's completed totals (what the overlay reads)
...
imgui_render_stats_publish( void )
{
    ...
    s_stats.accum     = ( imgui_render_stats_t ){ 0 };
}

// Peak draw-call count, read by the shutdown report in imgui_render.c.
static u32
...
/*----------------------------------------------------------------------------------------------
    ...
    correct.  Toggled via imgui()->set_retained_skip (key C in sb_vulkan).
----------------------------------------------------------------------------------------------*/

static bool s_retained_skip = true;

void imgui_render_set_retained_skip( bool on ) { s_retained_skip = on; }
bool imgui_render_retained_skip   ( void )     { return s_retained_skip; }
...
/*----------------------------------------------------------------------------------------------
    ...
    same retained/geometry numbers are also live through imgui()->render_stats() in the perf overlay.
----------------------------------------------------------------------------------------------*/
...
// rect_empty (a zero-area clip) is defined in imgui_draw.c, included before this unit.

#define RENDER_MAX_CLIP_GROUPS  64   // distinct clip rects groupable within one window

static void
cache_tess_window( imgui_id_t win )
{
    const imgui_cmd_seg_t* segs = s_draw.segs;
    u32                    nseg = s_draw.seg_count;
...
cache_build_frame( void )
{
    ...
    // Retained stats -- published via imgui_render_stats().
