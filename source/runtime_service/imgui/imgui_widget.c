...
/*----------------------------------------------------------------------------------------------
    Numeric text inputs -- input_int, input_float, input_double, input_float2/3/4.

    Each widget presents a text field seeded with the formatted current value on focus gain.
    The user edits freely; on Enter or focus loss the text is parsed back via strtod (which
    handles both decimal and scientific notation, e.g. "1e+8" -> 1000000.0).

    input_int / _float / _double optionally show [-][+] step buttons at the right edge of
    the control rect when step != 0.  Ctrl held at click uses step_fast instead.  The buttons
    use IMGUI_ITEM_BUTTON_REPEAT so holding fires continuously.

    input_floatN lays N equal sub-boxes across the control track with no step buttons.

    Scratch buffer: one static slot (only one field can be focused at a time), keyed by id.
