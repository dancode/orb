/*==============================================================================================
    ...
    Boot sequence:
        ...
        5. cache typed pointers
            to engine modules (app, render)
        6. desc->on_ready()                     — host post-init hook
        7. enter loop per desc->loop_mode
        8. mod_system_exit()                    — exit in reverse dep order

    The loop is intentionally explicit. rt_host.c knows the modules it manages
    and calls them by name. It does not iterate the dep graph generically.

    API slot stability
    ------------------
    mod_get_api() returns a pointer to the module's stable api_slot — a block the
    system owns and updates in-place on every hot-reload. s_app and s_render cached
    here never need refreshing; the function pointers they point to are live after
    every reload flush.
...
#include "engine/app/app.h"           /* pump_events — windowed hosts */
...
// MOD_DEFINE_API_PTR( app_api_t, app ); // skip because static api (never referenced)
// MOD_DEFINE_API_PTR( render_api_t, render ); -- uncomment when render_api.h exists 
...
rt_host_main( const rt_host_desc_t* desc, int argc, char** argv )
{
    ...
    /*
       HOST_FETCH_API in static builds:  no-op — app_api() returns &g_app_api_struct.
       HOST_FETCH_API in dynamic builds: g_app_api_ptr = mod_get_api("app").
                                         NULL when the module is absent (headless hosts).
    */
    ...
    while ( !g_quit_requested )
    {
        ...
        if ( app_api() && !app_api()->pump_events() )
        {
        ...
        // if ( s_render ) s_render->draw( dt ); -- add when render exists 
