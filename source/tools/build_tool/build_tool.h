...
typedef struct target_info_s
{
    ...
    /*  Runtime-Loaded Modules: modules this target loads at run time in a modular build and
        links as static libs in a monolithic one (BUILD_STATIC).

        The LINK half is monolithic-only -- ctx->is_monolithic gates it in build_target_link().
        The BUILD ORDER half applies in both modes: naming a module here means including its
        API header, and a module whose API struct and gateway are generated by reflect_tool
        (REF_MODULE) has no such header until it builds. */
