...
// find_job() keeps one job per target name and every job's target comes from
// g_targets[], so a closure can never hold more jobs than there are targets.
#define MAX_JOBS       MAX_TARGETS
#define MAX_THREADS    32   // Worker thread cap (further clamped to logical CPUs).
#define MAX_REV_DEPS   96   // Dependents per target. Inverse fan-out limit.
...
add_job( target_info_t* t )
{
    ...
    // Mono deps order the build in BOTH modes, though only a monolithic link consumes
    // their .lib. A host names a module here because it loads it, which means it includes
    // the module's API header -- and for a module whose API struct and gateway come from
    // reflect_tool (REF_MODULE), that header is generated by the module's own build. Order
    // it in modular builds too or the host can compile before the header exists.
