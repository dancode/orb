...
parse_buffer( const char* buf, rg_parse_data_t* out )
{
    ...
    while ( ( p = strchr( p, 'R' ) ) != NULL )
    {
        ...
        if      ( strncmp( s, "STRUCT", 6 ) == 0 && !is_ident_char( (unsigned char)s[ 6 ] ) ) { kind = RG_KIND_STRUCT; marker_len = 9; }
        else if ( strncmp( s, "BITSET", 6 ) == 0 && !is_ident_char( (unsigned char)s[ 6 ] ) ) { kind = RG_KIND_BITSET; marker_len = 9; }
        else if ( strncmp( s, "ENUM",   4 ) == 0 && !is_ident_char( (unsigned char)s[ 4 ] ) ) { kind = RG_KIND_ENUM;   marker_len = 7; }
...
rg_parse( const rg_file_list_t* files, rg_parse_data_t* out )
{
    ...
    for ( int i = 0; i < files->count; i++ )
    {
        ...
        int before = out->type_count;    // type count before parsing.

        /* Note: parse_buffer may modify out->type_count, out->header_count, and out->headers as
           it finds types and decides which files to include. We check if new types were added after parsing. */
