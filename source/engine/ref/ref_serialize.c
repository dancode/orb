/* engine/ref/ref_serialize.c - Binary read/write gated on schema_hash.
   ...
   ref_read validates magic, name_hash, schema_hash, and body_size before memcpy, refusing
   any buffer that does not exactly match the registered type layout.

   Header layout (all fields little-endian u32):
       [0..3]   REF_SAVE_MAGIC  (0x31307372 = 'rs01')
       [4..7]   type name_hash
...
static void ref_write_u32_le( uint8_t* dst, uint32_t v )
{
    dst[0]=(uint8_t)(v); dst[1]=(uint8_t)(v>>8); dst[2]=(uint8_t)(v>>16); dst[3]=(uint8_t)(v>>24);
}

static uint32_t ref_read_u32_le( const uint8_t* src )
{
...
static void
ref_serialize_zero_ptr( void** slot, uint16_t pointee_id, const ref_field_t* f, void* user )
{
...
static void
ref_serialize_zero_transient( uint8_t* body, uint16_t type_id )
{
...
size_t
ref_write( const void* instance, uint16_t type_id, uint8_t* buf, size_t cap )
{
    ...
    ref_write_u32_le( buf +  0, REF_SAVE_MAGIC );
    ref_write_u32_le( buf +  4, t->name_hash );
    ref_write_u32_le( buf +  8, t->schema_hash );
    ref_write_u32_le( buf + 12, (uint32_t)t->size );
    ref_write_u32_le( buf + 16, 0 );

    /* Copy the instance verbatim into the body, then sanitize volatile data in-place. */
    uint8_t* body = buf + REF_SAVE_HEADER_SIZE;
    memcpy( body, instance, t->size );

    ref_walk_refs( body, type_id, ref_serialize_zero_ptr, NULL );   /* zero pointer slots */
    ref_serialize_zero_transient( body, type_id );                 /* zero @transient fields */

    return need;
}

ref_io_status_t
ref_read( void* instance, uint16_t expected_type_id,
         const uint8_t* buf, size_t cap, size_t* bytes_read )
{
    if ( !instance || !buf ) return REF_IO_BAD_ARG;
    if ( cap < REF_SAVE_HEADER_SIZE ) return REF_IO_TRUNCATED;

    uint32_t magic       = ref_read_u32_le( buf +  0 );
    uint32_t type_hash   = ref_read_u32_le( buf +  4 );
    uint32_t schema_hash = ref_read_u32_le( buf +  8 );
    uint32_t body_size   = ref_read_u32_le( buf + 12 );

    /* Validate in order of cheapest check first:
       magic -> name identity -> field layout -> body size -> buffer capacity. */
    if ( magic != REF_SAVE_MAGIC ) return REF_IO_INCOMPAT;

    const ref_type_t* t = ref_get_type( expected_type_id );
    if ( !t ) return REF_IO_NO_TYPE;

    if ( t->name_hash   != type_hash   ) return REF_IO_INCOMPAT;   /* wrong type entirely */
    if ( t->schema_hash != schema_hash ) return REF_IO_INCOMPAT;   /* layout changed (hot-reload / version) */
    if ( body_size      != t->size     ) return REF_IO_INCOMPAT;   /* sizeof mismatch */

    if ( cap < (size_t)REF_SAVE_HEADER_SIZE + body_size ) return REF_IO_TRUNCATED;

    memcpy( instance, buf + REF_SAVE_HEADER_SIZE, body_size );

    if ( bytes_read ) *bytes_read = (size_t)REF_SAVE_HEADER_SIZE + body_size;
    return REF_IO_OK;
}

uint32_t
ref_peek_type_hash( const uint8_t* buf, size_t cap )
{
    ...
    if ( !buf || cap < REF_SAVE_HEADER_SIZE ) return 0;
    if ( ref_read_u32_le( buf + 0 ) != REF_SAVE_MAGIC ) return 0;
    return ref_read_u32_le( buf + 4 );
}
