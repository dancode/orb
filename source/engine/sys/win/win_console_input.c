...
{
    /* Map a Win32 virtual key code to our internal sys_key_t. 
       Returns PLATFORM_KEY_NONE if unmapped. */
...
/*============================================================================================*/
/* Internal state for console input handling. */

typedef struct sys_console_input_state_s
{
    HANDLE input_handle; /* console input handle (from GetStdHandle) */

    DWORD  original_console_mode;     /* saved console mode to restore on shutdown */
    bool   has_original_console_mode; /* whether original_console_mode is valid (set on init) */
    bool   initialized;               /* whether the console input system is initialized */

    bool   key_down[ PLATFORM_KEY_COUNT ];     /* current key states (true if currently down) */
    bool   key_pressed[ PLATFORM_KEY_COUNT ];  /* keys that transitioned from up to down since last poll */
    bool   key_released[ PLATFORM_KEY_COUNT ]; /* keys that transitioned from down to up since last poll */

} sys_console_input_state_t;

/* Single global instance since we only have one console. */
