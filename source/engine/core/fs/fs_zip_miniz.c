/*==============================================================================================
    ...
    engine/core/fs/fs_zip_miniz.c -- implementation TU for the vendored miniz amalgamation.

    Compiled as its own object in the core target (NOT folded into core.c's unity build) so
    miniz's ~10k lines and its file-scope statics stay isolated from the engine's own code.
    fs.c (inside core's unity TU) calls the mz_zip_* reader API declared in vendor/miniz.h; the
    definitions live here.  fs_zip.h sets the shared miniz configuration for both sides.

    Vendored miniz 3.0.2 (public domain / MIT); see vendor/miniz_LICENSE.txt.

==============================================================================================*/

#include "engine/core/fs/fs_zip.h"   /* MINIZ_NO_STDIO -- must precede the amalgamation */
