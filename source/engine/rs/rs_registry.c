    ...
    if ( field_count > 0 )
    {
        ...
        for ( uint16_t i = 0; i < field_count; i++ )
        {
            ...
            *dst            = fields[ i ];
            ...
            /* Best-effort resolve; remaining forward refs patched in rs_finalize_frame. */
...
rs_finalize_frame( uint16_t frame_id )
{
    ...
    for ( uint16_t i = f->first_field; i < field_end; i++ )
    {
        ...
        if ( fld->type_id != RS_TYPE_INVALID ) continue;
        if ( fld->type_hash == 0 ) continue;
...
rs_register_module( const char* name, const mod_desc_t* desc )
{
    ...
    rs_ensure_init(); /* decouple registry setup from mod_init ordering */
...
rs_unregister_module( const char* name )
{
    ...
    /* Module had no frame (no reflection) — silently ignore.
       Only warn if the name exists deeper in the stack, which would be a genuine LIFO violation. */
    if ( rs_debug )
    {
