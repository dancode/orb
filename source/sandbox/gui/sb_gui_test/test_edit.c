/*==============================================================================================
    ...
    Runs over the synthetic font from test_font.c (advance == codepoint & 0xFF), so every
    expected pixel names exactly which glyphs were measured.
...
{
    /* "a e-acute euro smiling-face" -- 1-, 2-, 3-, and 4-byte sequences in one run.
       Advances (== codepoint & 0xFF): 97, 233, 172, 66.
       Byte boundaries: 0, 1, 3, 6, 10.  Cumulative x: 0, 97, 330, 502, 568. */
    const char* s   = "a\xC3\xA9\xE2\x82\xAC\xF0\x9F\x99\x82";
    u32         len = 10;
    font_case_activate();
    ...
    test_true( text_x_at( s, 6 )  == 502.0f );
    test_true( text_x_at( s, 10 ) == 568.0f );
    ...
    test_true( text_x_at( s, 8 ) == 568.0f );

    /* Click-to-caret: snaps at each glyph's midpoint and returns ONLY sequence boundaries.
       Midpoints: 'a' 48.5, e-acute 213.5, euro 416, smiling-face 535. */
    ...
    test_equal( 3u,  text_offset_at( s, len, 415.0f ) );     /* left half of euro          */
    test_equal( 6u,  text_offset_at( s, len, 416.0f ) );     /* right half of euro         */
    test_equal( 6u,  text_offset_at( s, len, 534.0f ) );     /* left half of smiling-face  */
    test_equal( 10u, text_offset_at( s, len, 535.0f ) );     /* right half of smiling-face */
