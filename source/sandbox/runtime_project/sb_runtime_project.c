/*==============================================================================================
    ...
    sb_runtime_project.c -- the BYPASS PROOF: a custom host driving the project contract
    ...
    loads and hot-reloads sample_raw.dll (project_name in the descriptor) exactly as it
...
raw_host_ready( void )
{
    ...
    if ( !s_project )
    {
        fprintf( stderr, "[sb_runtime_project] project '%s' has no api\n", SB_PROJECT );
        run_host_quit();
        return;
    }

    printf( "[sb_runtime_project] driving '%s' directly -- no game framework loaded\n", SB_PROJECT );
...
raw_host_update( f32 dt )
{
    ...
    {
        printf( "[sb_runtime_project] Q -- quit\n" );
        if ( s_project )
            s_project->on_stop();
        run_host_quit();
        return;
    ...
    {
        printf( "[sb_runtime_project] R -- reload all\n" );
        mod_reload_all();
    }
