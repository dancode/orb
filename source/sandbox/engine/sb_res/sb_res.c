/*==============================================================================================
    ...
    Proves the properties the rest of the design leans on:
      ...
      - paths:         a table carries each name's cooked path; a by-name registration has
                       none until a table supplies it; two tables disagreeing keep the first
      ...
      - harvest:       this executable's own table, generated by the build from the RID()
                       literals in this file and resolved against content/, arrives through
                       res's descriptor with no hand-written registration -- the build-time
                       half of the design.  The fixtures live in content/sandbox/res/.
...
test_identity( void )
{
    ...
    rid_t a = RID( "sandbox/res/icon/save" );
    rid_t b = RID( "sandbox/res/icon/save" );
    sb_check( a == b, "same literal -> same id" );
    sb_check( a != RID_INVALID, "id is never RID_INVALID" );
    sb_check( a != RID( "sandbox/res/icon/load" ), "different names -> different ids" );
    ...
    sb_check( RID( "Sandbox/Res/Icon/Save" ) == a, "case folds" );
    sb_check( RID( "sandbox\\res\\icon\\save" ) == a, "backslash folds to slash" );
    sb_check( RES_TREE( "sandbox/res/icon" ) == res_hash_name( "sandbox/res/icon/" ),
    ...
    rid_t tree = RES_TREE( "sandbox/res/icon" );
    rid_t res  = res_hash_name( "sandbox/res/" );
    ...
    sb_check( res_hash_child( tree, "load" ) == res_hash_name( "sandbox/res/icon/load" ),
              "different leaf -> different child" );
    sb_check( res_hash_child( res, "icon/save" ) == a, "leaf may carry separators" );
    sb_check( res_hash_child( res, "icon/" ) == tree, "leaf with a slash is a nested subtree" );
    ...
    rid_t r = res_register( "Sandbox\\Res\\Icon\\Save" );
    sb_check( r == a, "register returns the hashed id" );
    sb_check( res_exists( a ), "registered id exists" );
    sb_check( str_eq( res_name( a ), "sandbox/res/icon/save" ), "name reads back canonical" );
...
test_idempotent( void )
{
    ...
    rid_t first = res_register( "sandbox/res/font/mono/16" );
    u32   n     = res_count();

    /* A reloaded DLL registers its table again: same ids, no growth. */
    for ( int i = 0; i < 3; ++i )
    {
        rid_t again = res_register( "sandbox/res/font/mono/16" );
    ...
    rid_t held = RID( "sandbox/res/font/mono/16" );
...
test_pool_copies( void )
{
    ...
    sb_check( str_eq( name, "shader/gui_quad" ), "name text is the original, not the scribble" );
...
test_growth( void )
{
    ...
    sb_check( str_eq( oldest, "growth/level_0000/mesh" ), "the oldest name survives every growth" );
...
test_collision( void )
{
    ...
    sb_check( str_eq( res_name( id ), "collide/2ae/40b" ), "first registration stands" );
...
/*==============================================================================================
    ...
    Nothing in this file registers "sandbox/res/icon/never" by hand -- test_misses only ever
    asks for it -- yet the name is in g_host_res_table because res_tool found the literal.
    That is the property the whole design rests on: what CAN be loaded is known at build
    time.  The names the fake module registers are spelled only as plain strings in its
    table, never through RID(), so the harvest must NOT contain them.  The one RES_TREE()
...
test_harvest( void )
{
    ...
    sb_check( res_exists( RID( "sandbox/res/icon/never" ) ), "a name only ever queried was still harvested" );
    sb_check( res_exists( RID( "sandbox/res/icon/save" ) ), "a name spelled in several cases harvested once" );
    sb_check( res_exists( RID( "sandbox/res/font/mono/16" ) ), "the held-id name was harvested" );
