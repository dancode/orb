...
> the build's content phase from the resource manifests into `build/content/shader/` (every
> build recooks a stale pair with the asset_tool on disk; `-content` builds the cooker first);
> gui reads the pair through fs and `render_init` fails loudly when it is missing. The draw
