...
The render backend is a three-phase pipeline, documented at the top of `gui_build_cache.c:5-18`
and `gui_submit_render.c:5-25`:

```
EMIT   (gui_emit_draw.c)     widgets push semantic shapes -> s_draw command list
BUILD  (gui_build_cache.c)   diff against last frame + tessellate -> s_tess geometry
SUBMIT (gui_submit_render.c) upload + issue indexed draw calls, once per live surface
```

**EMIT.** Widgets call `draw_push_rect_filled`, `draw_push_rect_outline`, `draw_push_triangle`,
`draw_push_circle_filled`, `draw_push_text[_n/_clip_n]`, and (from `gui_emit_path.c`)
`gui_draw_line`/`gui_draw_polyline`/`gui_draw_dashed_line`. Each push appends one semantic
`gui_cmd_t` to `s_draw.cmds[]` (`gui_emit_draw.c:50`) -- not GPU geometry yet. Ambient state folded in
at push time: a clip stack (`gui_emit_draw.c:217-245`), and alpha/rounding/text-clip multipliers
(`draw_set_alpha`, `draw_set_rounding`, `draw_set_text_clip_x`). Commands are partitioned into
**segments** tagged `(win, z, vp, font)` by `draw_seg_retag` (`gui_emit_draw.c:267-303`) so the BUILD
...
cache diff (`draw_hash_cmd`, `gui_emit_draw.c:497-537`). Every push also culls against the active clip
before it's stored (`draw_cull_box`) -- a culled push returns silently with no signal to the
caller.

**BUILD**, once per frame (`cache_build_frame`, `gui_build_cache.c:408-617`):
...
2. Each window has one **geometry slot** in a ping-pong table (`s_slots_a`/`s_slots_b`, swapped
   ...
   `tess_dispatch()`.
3. `tess_dispatch()` (`gui_build_tess.c:668-763`) walks the ordered commands and dispatches each
...
**Fonts.** Three deliberately distinct concepts (`gui_load_font.h:14-24`): a compile-time 1bpp *bit
...
**Debug tooling** (`gui_debug_overlay.c`, compiled only under `GUI_DEBUG_OVERLAY`) is an entirely separate
...
until `gui_render_flush` (`gui_emit_draw.c:688`), but the string is actually copied into a pool at push
time (`gui_emit_draw.c:714-722`) -- the comment is simply wrong, in the safe direction, but still
wrong.
